...
step 'On stage details page material tab for <pipeline_name> label <label> stage name <stage_name> counter <counter>' do |pipeline_name, label, stage_name, counter|
  stage_details_page.load(pipeline_name: scenario_state.get_pipeline(pipeline_name), label: label, stage_name: stage_name, counter: counter, tab_name: 'materials')
end
...
step 'Verify modification <modification_number> has revision <revision> - Already On Build Cause Section' do |modification_number, revision|
  ...
  if material_type == "Pipeline"
    ...
    actual_revision = [scenario_state.get_pipeline(upstream_pipeline), revision_arr.join('/')].join('/')
  end
